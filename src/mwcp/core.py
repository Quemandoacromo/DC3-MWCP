...
def run(
        ...
        *,
        ...
        include_file_data: bool = None,
        prefix_output_files: bool = None,
        external_strings_report: bool = None,
    ...
    if include_file_data is None:
        include_file_data = settings.report.include_file_data
    if prefix_output_files is None:
        prefix_output_files = settings.report.md5_prefix
    if external_strings_report is None:
        external_strings_report = settings.report.string_report
    if include_logs is None:
        include_logs = settings.report.include_logs
    ...
    report_config = dict(
        ...
        knowledge_base=knowledge_base or settings.knowledge_base,
    )
    if not yara_repo:
        yara_repo = settings.get("yara_repo")
