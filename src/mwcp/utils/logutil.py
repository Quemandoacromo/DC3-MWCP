...
class DemoteToDebug(logging.Filter):
    """
    Demotes all logs to DEBUG level.
    Useful to silence loud libraries without removing their logs.
    """

    def __init__(self, logger_name: str, level: int = logging.WARN):
        """
        :param logger_name: Logger to demote.
        :param level: Highest logging level to demote.
        """
        super().__init__()
    ...
    def filter(self, record: logging.LogRecord) -> bool:
        # apply only to chosen library
        if not record.name.startswith(self.logger_name):
            return True

        # demote record to DEBUG level if within highest requested level.
        if logging.DEBUG < record.levelno <= self.level:
...
class MPRotatingFileHandler(logging.handlers.RotatingFileHandler):
    ...
    def __init__(self, filename, **kwargs):
        ...
        if not os.path.isabs(filename):
            filename = os.path.join(platformdirs.user_log_dir("mwcp", appauthor="dc3"), filename)
...
def setup_logging(default_level=logging.INFO, queue=None):
    ...
    if queue:
        ...
        if log_config := settings.get("log_config_path"):
            try:
