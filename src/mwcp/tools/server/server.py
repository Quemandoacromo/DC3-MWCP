...
def _run_parser(parser_name: Optional[str], data: bytes) -> Report:
    ...
    try:
        ...
        knowledge_base = {**settings.knowledge_base, **_parse_parameters(params)}

        include_logs = _get_option("include_logs", settings.report.include_logs)
        report = mwcp.run(
            parser_name,
            data=data,
            recursive=_get_option("recursive", settings.recursive),
            ...
            external_strings_report=_get_option("external_strings", settings.report.string_report),
