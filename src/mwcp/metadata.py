...
from typing import Any, Union, List, Optional, TypeVar, Type, Dict, Self
...
class _MetadataConverter(cattr.Converter):
    """
    Custom cattrs converter that allows us to define structure/unstructure functions
    in the attrs fields.
    """

    def _structure_attribute(self, field, value):
        meta = getattr(field, "metadata", {})
        if (func := meta.get("structure")) and value is not None:
            return func(value)
        return super()._structure_attribute(field, value)

    def structure_attrs_fromdict(self, obj: dict, klass):
        # Strip out init=False fields or None fields before processing.
        fields = attr.fields_dict(klass)
        new_obj = {}
        for key, value in obj.items():
            if value is not None and fields[key].init:
                new_obj[key] = value
        return super().structure_attrs_fromdict(new_obj, klass)

    # NOTE: There is no _unstructure_attribute, so we have to redfine the whole thing.
    def unstructure_attrs_asdict(self, obj: Any) -> dict[str, Any]:
        ...
        for a in fields:
            ...
            if (func := meta.get("unstructure")) and v is not None:
                rv[name] = func(v)
            else:
                rv[name] = dispatch(a.type or v.__class__)(v)
        return rv

cattr = _MetadataConverter()
...
cattr.register_structure_hook(bytes, lambda d, t: base64.b64decode(d) if isinstance(d, str) else d)
cattr.register_unstructure_hook(bytes, lambda d: None if d is None else base64.b64encode(d).decode())

# Convert sets to sorted list.
cattr.register_unstructure_hook(set, sorted)

# Convert UUID
cattr.register_unstructure_hook(uuid.UUID, str)

# Field used to serialize large integers as base64.
bigint_field = partial(
    attr.ib,
    metadata={
        "jsonschema": {
            "type": "string",
        ...
        "structure": (
            lambda data: int.from_bytes(base64.b64decode(data), byteorder="big")
        ),
        "unstructure": (
            lambda value: base64.b64encode(value.to_bytes((value.bit_length() + 7) // 8, byteorder="big")).decode()
        ),
    }
)
...
def _cast(value: Any, type_: Type[T]) -> T:
    ...
    if type_ is str and isinstance(value, (bytes, bytearray)):
...
class Element:
    ...
    def _schema(cls, extractor):
        ...
        for field in cls.fields():
            ...
            if "jsonschema" in field.metadata:
                ...
                if callable(sub_schema):
                    sub_schema = sub_schema(cls)
            else:
    ...
    def from_dict(cls, value: dict) -> "Self":
        warnings.warn(
            ...
            "Please use ** operator in the constructor or from_json_dict() for deserialization.",
            DeprecationWarning
        )
        return cls.from_json_dict(value)

    @staticmethod
    def _as_dict_aux(instance, key, value):
        if isinstance(value, Element):
            value = value.as_dict()
        return value

    def as_dict(self, flat=False) -> dict:
        ret = attr.asdict(self, retain_collection_types=True, value_serializer=self._as_dict_aux)
        if flat:
    ...
    def as_json_dict(self, flat=False) -> dict:
        """
        Provides a json serializable dictionary of the element.
    ...
    def from_json(cls, data: str) -> "Self":
        """
        Loads json text into Element.
        """
        return cls.from_json_dict(json.loads(data))

    @classmethod
    def from_json_dict(cls, obj: dict) -> "Self":
        """
        Converts json serializable dictionary into metadata instance.
        """
    ...
    def post_processing(self, report):
        ...
        """
        # Add the tags of each element to all of its sub-elements.
        for element in self.elements():
            element.add_tag(*self.tags)
...
class Path2(Metadata):
    ...
    def as_stix(self, base_object, fixed_timestamp=None) -> STIXResult:
        result = STIXResult(self, fixed_timestamp=fixed_timestamp)
...
class Alphabet(Metadata):
    ...
    def as_stix(self, base_object, fixed_timestamp=None) -> STIXResult:
        ...
        return STIXResult(self, content)
...
class ScheduledTask(Metadata):
    ...
    actions: _ActionType = attr.ib(
        ...
        metadata={
            ...
            # Giving some help because cattrs has trouble with Unions.
            "structure": lambda data: [cattr.structure_attrs_fromdict(command, Command) for command in data],
        }
    )
    name: str = None
...
class Other(Metadata):
    ...
    def as_stix(self, base_object, fixed_timestamp=None) -> STIXResult:
        ...
        if self.value_format in ("boolean", "integer") or self.value in (b"", ""):
            ...
            result = STIXResult(self, content)
        else:
            content = {
...
class RSAPrivateKey(Metadata):
    ...
    modulus: int = bigint_field(default=None)
    private_exponent: int = None
    p: int = bigint_field(default=None)
    q: int = bigint_field(default=None)
    d_mod_p1: int = bigint_field(default=None)
    d_mod_q1: int = bigint_field(default=None)
    q_inv_mod_p: int = bigint_field(default=None)
...
class File(Metadata):
    ...
    _MD5_RE = re.compile("[a-fA-F0-9]{32}")
    _SHA1_RE = re.compile("[a-fA-F0-9]{40}")
    _SHA256_RE = re.compile("[a-fA-F0-9]{64}")
    ...
    @md5.validator
    def _validate_md5(self, attribute, value):
        if value and not self._MD5_RE.fullmatch(value):
            raise ValidationError(f"Invalid MD5 hash found: {value!r}")

    @sha1.validator
    def _validate_sha1(self, attribute, value):
        if value and not self._SHA1_RE.fullmatch(value):
            raise ValidationError(f"Invalid SHA1 hash found: {value!r}")

    @sha256.validator
    def _validate_sha256(self, attribute, value):
        if value and not self._SHA256_RE.fullmatch(value):
            raise ValidationError(f"Invalid SHA256 hash found: {value!r}")
    ...
    def write(self, directory: pathlib.Path = None, prefix=True) -> pathlib.Path:
        """
        Writes out File metadata object to the given output directory.

        :param directory: Path to write out file. (defaults to current directory.)
        :param prefix: Whether to prefix the filename with the md5 hash.
        """
        if self.data is None:
            raise ValueError(f"Missing data to write {self}")

        directory = pathlib.Path(directory or ".")

        # Create a safe filename that won't have any name collisions.
        safe_filename = sanitize_filename(self.name or self.md5)
        if prefix:
            safe_filename = f"{self.md5[:5]}_{safe_filename}"
