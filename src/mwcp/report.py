...
from typing import Union, Iterable, Optional, Type, List, Callable, TypeVar, NamedTuple
import weakref
...
class ReportLogHandler(logging.Handler):
    ...
    def emit(self, record):
        if report := self._report_ref():
            ...
            report.add_log(message, level=record.levelno)
...
class Report:
    ...
    def __init__(
        ...
        self.prefix_output_files = prefix_output_files
    ...
    def fields(self) -> set:
        ...
        fields_path = pathlib.Path(mwcp.config.__path__[0]) / "fields.json"
    ...
    def unidentified(self) -> List[FileObject]:
        ...
        if not self.input_file:
            return []
        ret = []
        for file_object in [self.input_file] + self.input_file.descendants:
            if file_object.parser in (UnidentifiedFile, None):
    ...
    def add_log(self, msg: str, level: int = logging.INFO, source: Optional[FileObject] = None):
        """
        Add a new log message to the report.
        NOTE: Usually this is done automatically with a setup log handler.
        """
        if not source:
            source = self._current_file
        self._logs.append(LogRecord(source, level, msg))
    ...
    def _load_report_model(self, report_model: ReportModel, source: FileObject = None):
        """
        Loads report metadata element into the report object.
        """
        if not source:
            source = self._current_file
        with self.parsing_file(source):
            for message in report_model.logs:
                level = logging.INFO
                if match := re.search(r"^\[([!\-+* ])\]", message):
                    level = {
                        ...
                        "+": logging.INFO,
                        "*": logging.DEBUG,
                    }.get(match.group(1), logging.INFO)
                self.add_log(message, level=level)

            for entry in report_model.metadata:
                if isinstance(entry, metadata.File):
                    # Construct a FileObject in order to get added to source as child.
                    FileObject.from_metadata(entry, parent=source)
                self.add(entry, validate=False, post_processing=False)

            self.add_tag(*report_model.tags)

    @property
    def _report_model(self) -> ReportModel:
        """
    ...
    @classmethod
    def from_json(cls, data: str, include_file_data=False) -> "Report":
        """
        Deserializes a json string into a Report.
        """
        return cls.from_json_dict(json.loads(data), include_file_data=include_file_data)

    @classmethod
    def from_json_dict(cls, obj: Union[dict, list], include_file_data=False) -> "Report":
        """
        Converts json serializable dictionary or list of dictionaries into Report instance.
        """
        if not obj:
            raise ValueError(f"Empty data: {repr(obj)}")

        if isinstance(obj, dict):
            report_model = ReportModel.from_json_dict(obj)
            input_file = FileObject.from_metadata(report_model.input_file)
            report = cls(
                input_file=input_file,
                parser=report_model.parser,
                recursive=report_model.recursive,
                knowledge_base=report_model.external_knowledge,
                include_logs=bool(report_model.logs),
                include_file_data=include_file_data and input_file.data is not None,
            )
            report._load_report_model(report_model)
            report.finalized = True
            return report

        elif isinstance(obj, list):
            report: Report = None
            for report_dict in obj:
                report_model = ReportModel.from_json_dict(report_dict)
                input_file = FileObject.from_metadata(report_model.input_file)
                if not report:
                    # First file is treated as the initial input file.
                    report = cls(
                        input_file=input_file,
                        parser=report_model.parser,
                        recursive=report_model.recursive,
                ...
                report._load_report_model(report_model, source=input_file)
            report.finalized = True
            return report

        raise TypeError(f"Unexpected data: {repr(obj)}")
    ...
    def add(self, element: Metadata, validate=True, post_processing=True):
        """
        ...
        :param validate: Validates the metadata element.
        :param post_processing: Whether to post-process the metadata element.
            This allows for supplemental elements to be automatically added.
            (e.g. adding FilePath from a path found within a Service element)
    ...
    def parsing_file(self, file_object: FileObject):
        """
        Temporarily sets the given file_object as the being parsed.
        """
        current_file = self._current_file
        self.set_file(file_object)
        try:
            yield
        finally:
            self.set_file(current_file)
    ...
    def output_file(self, data: bytes, filename: str = None, description: str = None):
        ...
        if self._write_output_files:
            return residual_file.write(self._output_directory, self.prefix_output_files)
    ...
    def finalize(self):
        ...
        if self._write_output_files:
            for residual_file in self.iter(metadata.File):
                residual_file.write(self._output_directory, self.prefix_output_files)
