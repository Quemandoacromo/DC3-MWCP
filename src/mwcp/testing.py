...
class TestCase:
    ...
    def update(self, force: bool = False, recursive: bool = None, knowledge_base: dict = None) -> bool:
        ...
        if recursive and not settings.get("yara_repo", None):
            logger.warning(
...
def get_path_in_malware_repo(file_path: Union[str, pathlib.Path] = None, md5: str = None) -> pathlib.Path:
    ...
    """
    malware_repo = settings.get("malware_repo")
    if not malware_repo:
        raise ValueError("'malware_repo' configuration not setup. Try running `mwcp config` to set this.")
    if file_path:
...
def _get_testcase_dir_from_source(source: registry.Source) -> pathlib.Path:
    ...
    if source.is_pkg:
        ...
        ref = importlib.resources.files(source.path) / "tests"
...
def get_testcase_dir(source: registry.Source) -> pathlib.Path:
    ...
    if testcase_dir := settings.get("testcase_dir"):
        return pathlib.Path(testcase_dir, source.name).expanduser()
...
def iter_test_cases(source: registry.Source = None, parsers: List[str] = None) -> Iterable[TestCase]:
    ...
    # Test case directories are organized into subdirectories based on source name.
    if testcase_dir := settings.get("testcase_dir"):
        testcase_dir = pathlib.Path(testcase_dir).expanduser()
...
def iter_failed_tests() -> Iterable[TestCase]:
    ...
    """
    lastfailed_file = pathlib.Path(settings.pytest_cache_dir) / "v" / "cache" / "lastfailed"
    if lastfailed_file.exists():
        with open(lastfailed_file, "r") as fo:
            data = json.load(fo)
        for key, enabled in data.items():
            if enabled:
                identifier = re.search(r"\[(.*)\]", key).group(1)
