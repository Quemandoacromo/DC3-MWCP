...
def register_entry_points():
    ...
    :return:
    """
    for entry in importlib.metadata.entry_points(group="mwcp.parsers"):
        package = entry.load()
...
def _import_parser(name: str, source: Source):
    ...
    try:
        ...
        # Mark skipped if testing.
        if "PYTEST_CURRENT_TEST" in os.environ and settings.testing.skip_missing:
            import pytest
            pytest.skip(f"{module_fullname} missing dependency: {e}")
        raise
...
def _generate_parser(name: str, source: Source, recursive=True, _visiting=None) -> Dispatcher | Type[Parser]:
    """
