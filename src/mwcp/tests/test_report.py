...
def test_deserialization(filled_report):
    json_data = filled_report.as_json()
    report = mwcp.Report.from_json(json_data)
    assert report.as_json_dict() == filled_report.as_json_dict()
    assert report.as_text() == filled_report.as_text()
