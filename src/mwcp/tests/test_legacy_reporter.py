...
def test_output_file(tmpdir):
    ...
    with report:
        assert report.output_file(b'This is data!', 'foo.txt', description='A foo file') == test_file
        ...
        assert report.output_file(b'More data!', 'foo.txt', description='Another foo file') == test_file
    ...
    report = mwcp.Report(output_directory=str(tmpdir))
    with report:
        assert report.output_file(b'blah', u'héllo!!\x08.txt') == test_file
