...
def test_sub_element_tagging(filled_report):
    test_tag = "c2"
    network = metadata.Network(url=metadata.URL2(url="https://www.test.com/test"), socket=metadata.Socket(4000))
    network.add_tag(test_tag)
    assert test_tag in network.tags
    assert test_tag not in network.url.tags
    assert test_tag not in network.socket.tags

    # Sub elements should have tags added during post-processing.
    network.post_processing(filled_report)
    assert test_tag in network.tags
    assert test_tag in network.url.tags
    assert test_tag in network.socket.tags


def test_serialization():
    # Test simple metadata.
    p = metadata.Path2("C:\\hello\\world.txt").add_tag("download")
    p_dict = p.as_json_dict()
    assert p_dict == {
        'type': 'path',
        'tags': ['download'],
    ...
    """).strip()
    assert metadata.Path2.from_json_dict(p_dict) == p
    assert metadata.Metadata.from_json_dict(p_dict) == p
    ...
    """).strip()
    assert metadata.URL2.from_json_dict(u_dict) == u
    assert metadata.Metadata.from_json_dict(u_dict) == u
...
def test_other_serialization_issue():
    ...
    other_dict = other.as_json_dict()
    assert other_dict == {
        "type": "other",
        "tags": [],
        "key": "test",
        "value": "aGVsbG8=",  # b"hello"
        "value_format": "bytes",
    }
    assert metadata.Other.from_json_dict(other_dict) == other
...
def test_scheduled_task_from_xml():
    ...
    """)
    assert scheduled_task.as_json_dict() == {
        "type": "scheduled_task",
        "tags": [],
        "name": None,
        "author": "AuthorName",
...
def test_deserialization_with_inner():
    """
    Tests deserialization with inner metadata object.
    """
    task = metadata.ScheduledTask(actions=["calc.exe", "notepad.exe"])
    data = task.as_json_dict()
    assert data == {
        "type": "scheduled_task",
        "tags": [],
        "name": None,
    ...
    assert metadata.Element.from_json_dict(data) == task


def test_bigint():
    """
    Tests handling of fields with big integers.
    """
    item = metadata.RSAPublicKey(1,123456)
    assert item.as_json_dict() == {
        "type": "rsa_public_key",
        "tags": [],
        "public_exponent": 1,
        "modulus": "AeJA",
...
def test_deserialization(filled_report):
    """
    Tests deserializing a full report.
    """
    report = filled_report._report_model
    json_data = report.as_json()
    actual_report = metadata.Report.from_json(json_data)
