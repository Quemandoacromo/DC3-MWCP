...
def filled_report(report, metadata_items):
    ...
    with report:
        for item in metadata_items:
            if isinstance(item, metadata.File):
                mwcp.FileObject.from_metadata(item, parent=report.input_file)
            report.add(item)
...
def cleanup_logging_handlers():
    """
    Cleanup logging stream handlers.
    github.com/pytest-dev/pytest/issues/5502
    """
    try:
        yield
    finally:
