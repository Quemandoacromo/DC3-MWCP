...
def test_parse(tmp_path, datadir):
    ...
    with runner.isolated_filesystem(tmp_path):
        ...
        ret = runner.invoke(cli.main, ["parse", "--no-recursive", "foo", test_file])
        print(ret.stdout)
        ...
        ret = runner.invoke(cli.main, ["parse", "--no-recursive", "--no-output-files", "foo", test_file])
        assert ret.exit_code == 0
        # We should still not have the output file
        assert not output_file.exists()

        # Test the json formatting
        ret = runner.invoke(cli.main, ["parse", "--no-recursive", "--no-output-files", "-f", "json", "foo", test_file])
        print(ret.stdout)
        print(ret.stderr, file=sys.stderr)
        assert ret.exit_code == 0
