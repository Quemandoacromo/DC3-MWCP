...
def _setup(config):
    ...
    if testcase_dir := config.getoption("testcase_dir"):
        settings.testcase_dir = testcase_dir
    if malware_repo := config.getoption("malware_repo"):
        settings.malware_repo = malware_repo
    if yara_repo := config.getoption("yara_repo"):
        settings.yara_repo = yara_repo
    if keep_tmp := config.getoption("keep_tmp"):
        settings.keep_tmp = keep_tmp
    if full_diff := config.getoption("full_diff"):
        settings.testing.full_diff = full_diff
...
def _fixup_test_cases(expected_results, actual_results):
    ...
    if expected_results_version >= version.parse("3.7.0"):
        ...
        is_supplemental = lambda item: (
            item["type"] == "file"
            and item["description"] in ("IDA Project File", "Ghidra Project File", "Vivisect Project File")
        )
    ...
    # Version 3.15 changes serialization for RSA key fields.
    if expected_results_version < version.parse("3.15.0"):
        items = []
        for item in expected_results["metadata"]:
            if item["type"] == "rsa_public_key":
                tags = item.pop("tags")
                item.pop("type")
                item = mwcp.metadata.RSAPublicKey(**item).add_tag(*tags).as_json_dict()
            elif item["type"] == "rsa_private_key":
                tags = item.pop("tags")
                item.pop("type")
                item = mwcp.metadata.RSAPrivateKey(**item).add_tag(*tags).as_json_dict()
            items.append(item)
        expected_results["metadata"] = items
...
def _test_parser(pytestconfig, input_file_path, results_path):
    ...
    if recursive and "yara_repo" not in settings:
        pytest.fail(
