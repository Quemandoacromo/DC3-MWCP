...
class PublicKey(Parser):
    DESCRIPTION = "RSA Public Key"

    RSA_PUB_KEY_RE = re.compile(
        br"-----BEGIN PUBLIC KEY-----(?P<data>[^-]*)-----END PUBLIC KEY-----",
        re.DOTALL
    )

    @classmethod
    def identify(cls, file_object):
        return cls.RSA_PUB_KEY_RE.search(file_object.data) and all(
            c in string.printable.encode() for c in file_object.data
        )

    def run(self):
        for match in self.RSA_PUB_KEY_RE.finditer(self.file_object.data):
            self.report.add(metadata.RSAPublicKey.from_PEM(match.group(0).decode()))
